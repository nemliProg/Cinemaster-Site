...
<div class="content">
  ...
  <div class="container addPostContainer p-5">
    <div class="row formTitles">
      <h3 class="mb-0"><span>Edit Post</span><span id="point">.</span></h3>
      <p class="mb-3">contribute in our community</p>
    </div>
    <form action="<?php echo URLROOT; ?>/posts/edit/<?= $data['post_id'] ?>" enctype="multipart/form-data" method="post" class="form mt-8">
      ...
      <div class="row d-flex flex-column">
        <input type="file" name="image" class="form-control form-control-lg bg-white custom-file-input form_elem col-md col-sm-12 col-lg<?= (!empty($data['image_err'])) ? 'is-invalid' : ''; ?>" value="<?= $data['image']; ?>">
